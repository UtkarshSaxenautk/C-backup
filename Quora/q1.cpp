...
class User : public Story {
    public:
    int index ;
    int number_Of_stories ;
    Story *St ;
    St = new Story[number_Of_stories];
     User *creater ;
    GenericMyVector<User> *followers_of_story ;
   ...
   for(int i =0 ; i < s.size(); i++ ) {
       for(int j = 0 ; j < s[i].followers_of_story.size(); j++) {
           for(int k = 0 ; k < v.size(); k++) {
               if(v[i].index == s[i].followers_of_story[j].index){
                   return true ;
               }
           }
       }
...
int main() {
   ...
   int p , q ;
    ...
    while(q--) {
        ...
        s[y -1].followers_of_story.push_back(u[x -1]);
    }
    ...
    for(int i = 0 ; i < m ; i++) {
        ...
        for(int j = 0 ; j < n ; j++) {
             ...
             for(int k = 0 ; k < m ; k++) {
                ...
                else if(checkfollwingbyStory(u[i].Storyfollowed , u[k].StoryCreated)){
                    a = 2 ;
                }
